...
type PatternConfig struct {
	...
	DeviceGroup    int               `yaml:"device_group"`    // Optional: regex capture group for device name (e.g., /dev/ptp0)
	StateMapping   map[string]float64 `yaml:"state_mapping"` // Optional: map state strings to numeric values (e.g., {"s0": 10, "s1": 20})
	ValueMultiplier float64          `yaml:"value_multiplier"` // Optional: multiply extracted value by this factor (e.g., 0.001 to convert ps to ns)
	ConvertNanosecondOffset bool     `yaml:"convert_nanosecond_offset"` // Optional: if true, convert fractional nanoseconds >= 500000000 to negative offset
...
type VisualizationConfig struct {
	...
	DPI        int             `yaml:"dpi"`
	YMin       *float64        `yaml:"y_min"`       // Optional: minimum Y-axis value
	YMax       *float64        `yaml:"y_max"`       // Optional: maximum Y-axis value
	YRange     *float64        `yaml:"y_range"`     // Optional: symmetric Y-axis range (+/- value)
	YTickSpacing *float64      `yaml:"y_tick_spacing"` // Optional: spacing between Y-axis ticks (e.g., 50 for ticks every 50 units)
	YTickCount   *int          `yaml:"y_tick_count"`   // Optional: number of Y-axis ticks (alternative to spacing)
